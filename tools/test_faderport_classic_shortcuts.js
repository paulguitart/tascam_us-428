...
assert.strictEqual(kc.firstSend.mLevel.getProcessValue(kd), kc.SEND_HOST_UNITY)
assert.deepStrictEqual(knob.commands.splice(0), ['Zoom/Zoom Out'])
knob.tap('btnMix') // Explicit knob mode overrides OUTPUT zoom.
...
assert.strictEqual(locked.getProcessValue(md), 0, 'OUTPUT selects zoom and releases PROJ')
mouse.tap('btnProject'); mouse.idle()
assert.strictEqual(locked.getProcessValue(md), 1)
mouse.tap('btnProject')
assert.strictEqual(locked.getProcessValue(md), 0)
...
const holdSave = load(), hs = holdSave.ctx, hd = holdSave.d
holdSave.tap('btnStop'); holdSave.idle(2000)
assert.deepStrictEqual(holdSave.commands.splice(0), ['Transport/Stop'])
holdSave.press('btnStop')
holdSave.midi.length = 0
holdSave.idle(499)
assert.strictEqual(holdSave.midi.length, 0, 'No progress during first half second')
holdSave.idle(1)
assert.deepStrictEqual(holdSave.midi, [[0xA0, 3, 1], [0xA0, 4, 0], [0xA0, 1, 0], [0xA0, 0, 0]])
holdSave.press('btnStop') // Duplicate press must not restart the timer.
holdSave.idle(999)
assert.deepStrictEqual(holdSave.commands, ['Transport/Stop'])
holdSave.idle(1)
assert.deepStrictEqual(holdSave.commands.splice(0), ['Transport/Stop', 'File/Save'])
holdSave.midi.length = 0
hs.transportValues.mStart.mOnProcessValueChange(hd, {}, 1)
hs.transportValues.mRecord.mOnProcessValueChange(hd, {}, 0)
assert.strictEqual(holdSave.midi.length, 0, 'Host feedback must not interrupt save animation')
hs.transportValues.mStop.mOnProcessValueChange(hd, {}, 1)
hs.transportValues.mCycleActive.mOnProcessValueChange(hd, {}, 1)
assert.deepStrictEqual(holdSave.midi, [[0xA0, 2, 1], [0xA0, 8, 1]])
for (let i = 0; i < 9; ++i) holdSave.idle(140)
assert.strictEqual(hd.getState('classic.saveBlinkCount'), '')
assert.deepStrictEqual(holdSave.midi.slice(-4), [[0xA0, 4, 0], [0xA0, 3, 0], [0xA0, 1, 1], [0xA0, 0, 0]])
holdSave.idle(5000)
assert.deepStrictEqual(holdSave.commands, [], 'One save per held press')
holdSave.press('btnStop', 0)
holdSave.press('btnStop'); holdSave.idle(1500); holdSave.press('btnStop', 0)
assert.deepStrictEqual(holdSave.commands.splice(0), ['Transport/Stop', 'File/Save'])
for (let i = 0; i < 9; ++i) holdSave.idle(140)
holdSave.press('btnStop'); holdSave.idle(700); holdSave.press('btnStop', 0)
assert.strictEqual(hd.getState('classic.stopProgress'), '')
holdSave.idle(2000)
assert.deepStrictEqual(holdSave.commands.splice(0), ['Transport/Stop'])
holdSave.press('btnStop'); holdSave.idle(600); holdSave.tap('btnRew'); holdSave.idle(2000)
holdSave.press('btnStop', 0)
assert.deepStrictEqual(holdSave.commands.splice(0), ['Transport/Stop', 'Transport/Return to Zero'])
holdSave.press('btnRew'); holdSave.press('btnStop'); holdSave.press('btnRew', 0); holdSave.idle(2000)
holdSave.press('btnStop', 0)
assert.deepStrictEqual(holdSave.commands.splice(0), ['Transport/Stop'], 'REW held before STOP must not arm Save')
for (const deactivate of [() => holdSave.page.mOnDeactivate(hd), () => holdSave.driver.mOnDeactivate(hd)]) {
    holdSave.press('btnStop'); holdSave.idle(700); deactivate(); holdSave.idle(2000)
    assert.deepStrictEqual(holdSave.commands.splice(0), ['Transport/Stop'])
    holdSave.press('btnStop', 0)
    holdSave.driver.mOnActivate(hd); holdSave.page.mOnActivate(hd, holdSave.mapping)
}
holdSave.press('btnStop'); holdSave.tap('btnProject'); holdSave.idle(100)
assert.strictEqual(hd.getState('classic.mouseSavedValue') !== '', true, 'Mouse locking and hold timer share idle')
holdSave.idle(1400)
assert.deepStrictEqual(holdSave.commands.splice(0), ['Transport/Stop', 'File/Save'])
...
for (const startSend of [false, true]) {
    const outputZoom = load(), oz = outputZoom.ctx, ozd = outputZoom.d
    oz.selectedValues.mPan.setProcessValue(ozd, 0.2)
    oz.firstSend.mLevel.setProcessValue(ozd, 0.7)
    if (startSend) outputZoom.tap('btnMix')
    outputZoom.tap('btnOutput')
    assert.strictEqual(ozd.getState('classic.zoomMode'), '1')
    for (let i = 0; i < 250; ++i) oz.panKnobRaw.mOnProcessValueChange(ozd, 0.01)
    assert.deepStrictEqual(outputZoom.commands.splice(0), Array(250).fill('Zoom/Zoom In'))
    oz.panKnobRaw.mOnProcessValueChange(ozd, 0.99)
    oz.panKnobRaw.mOnProcessValueChange(ozd, 0.5) // Neutral/invalid direction produces no zoom.
    outputZoom.tap('btnTransport')
    assert.deepStrictEqual(outputZoom.commands.splice(0), ['Zoom/Zoom Out', 'Zoom/Zoom to Locators'])
    assert.strictEqual(oz.selectedValues.mPan.getProcessValue(ozd), 0.2)
    assert.strictEqual(oz.firstSend.mLevel.getProcessValue(ozd), 0.7)
    outputZoom.tap('btnOff')
    outputZoom.tap('btnTransport')
    oz.panKnobRaw.mOnProcessValueChange(ozd, 0.01)
    assert.deepStrictEqual(outputZoom.commands.splice(0), ['Zoom/Zoom to Locators', 'Zoom/Zoom In'])
    outputZoom.tap('btnOutput') // Re-enable output, preserving the return mode.
    outputZoom.tap('btnOutput') // Return to track and the previous knob target.
    assert.strictEqual(ozd.getState('classic.zoomMode'), '')
    assert.strictEqual(ozd.getState('classic.sendMode'), startSend ? '1' : '')
    outputZoom.tap('btnOutput'); outputZoom.tap('btnBank'); outputZoom.idle()
    assert.strictEqual(ozd.getState('classic.zoomMode'), '')
    assert.strictEqual(ozd.getState('classic.mouseFader'), '1')
    outputZoom.tap('btnOutput')
    assert.strictEqual(ozd.getState('classic.mouseFader'), '')
    assert.strictEqual(ozd.getState('classic.zoomMode'), '1')
    outputZoom.driver.mOnDeactivate(ozd); outputZoom.driver.mOnActivate(ozd)
    outputZoom.page.mOnActivate(ozd, outputZoom.mapping)
    assert.strictEqual(ozd.getState('classic.zoomMode'), '')
}
