...
- Encoder rotation uses the knob modes below. In either Master mode it controls horizontal zoom. Encoder push centers pan in Pan mode, toggles Send 1 pre/post-fader in SHIFT + Pan mode, zooms to locators in either Master mode, toggles Cubase's metronome in Click mode, resets high-pass cutoff to minimum in Channel mode, and inserts a marker in Marker mode.
...
In Scroll/Zoom, Section, and both Master modes, knob push runs `Zoom > Zoom to Locators`, once per press. Scroll/Zoom, Section, Marker, and both Master modes share horizontal zoom rotation. Zoom pulses commands for repeated movement and seeds its comparison value on mode entry to avoid an immediate zoom jump. Like the Korg pattern, reaching the normalized range endpoint may require reversing the knob before further travel is available; verify the relative encoder behavior in Cubase.
...
- `node tests_iostation/knob.test.js` checks knob and fader mode targets, SHIFT + Master FX Return 1 fader binding, zoom push in both Master modes, selectors, mode LEDs, marker controls, repeated zoom pulses and mode-entry baselines.
- `node tests_iostation/master-mode.test.js` checks Main Mix insert feedback in Master and FX Return mute toggling/LED feedback in SHIFT + Master.
