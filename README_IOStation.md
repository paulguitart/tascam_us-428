...
- With SHIFT off, Write/Read toggle the selected track's automation Write/Read enable. WRITE lights red and READ lights green while enabled; both follow track selection and Cubase edits in every mode. SHIFT + Write (Trim) and SHIFT + Read (Off) remain unassigned, as does SHIFT + TOUCH (Latch), except in LINK and Send modes. SHIFT + BYPASS (BypassAll) is unassigned outside modes that explicitly keep BYPASS active. With either SHIFT state, BYPASS toggles the shared remembered fader bypass in Pan, Scroll/Zoom, Section and Marker, selected-track Send 1 enable in Send mode (Pan knob push separately centers pan), and mirrors encoder push in Click (metronome on/off). In normal Master, BYPASS toggles Main Mix insert bypass; in SHIFT + Master, it toggles FX Return 1 mute. In Channel, BYPASS toggles high-pass on/off while knob push resets cutoff to minimum. Scroll, Section and Marker share PAN's fader-bypass flag; their knob-press actions stay unchanged. Only Click mode uses BYPASS for the metronome.
...
- Encoder rotation uses the knob modes below. In Click and either Master mode it controls horizontal zoom. Encoder push centers pan in Pan mode, toggles Send 1 pre/post-fader in SHIFT + Pan mode, zooms to locators in either Master mode, toggles Cubase's metronome in Click mode, resets high-pass cutoff to minimum in Channel mode, and inserts a marker in Marker mode.

Outside LINK modes, with SHIFT off, TOUCH resets the current fader target: selected-track or Stereo Out volume goes to 0 dB using `FADER_HOST_UNITY`; metronome level goes to maximum. TOUCH lights red at maximum, amber at minimum, and white at 0 dB (`FADER_HOST_UNITY`) for channel/output volume; otherwise it is off. Detection allows one 14-bit MIDI step of tolerance. Metronome maximum is red, including after a TOUCH reset; it has no white 0 dB indication. It follows host edits and track changes. Scroll, Section and Marker use selected-track volume, so TOUCH resets that track to 0 dB. SHIFT + TOUCH remains unassigned outside LINK and Send modes. In both LINK modes, TOUCH captures the mouse parameter and shows green at the saved value, magenta above it, or red-leaning magenta below it. Motor movement follows the existing touch protection.
...
| Click | Knob controls horizontal zoom; fader controls metronome level (`ENABLE_METRONOME_FADER = true`); encoder push or BYPASS toggles the metronome |
| Channel | Selected-track high-pass cutoff; knob push resets cutoff to minimum; BYPASS toggles the filter |
...
In normal Master, the BYPASS LED follows `mBypass` for insert slot 1 (`accessSlotAtIndex(0)`) through the `Stereo Out Insert 1` viewer on the first output channel. It lights when that slot is **not bypassed**. This reports slot 1's state, while the button command targets all Main Mix inserts. In SHIFT + Master, the BYPASS LED follows the first FX channel's `mMute` and lights when FX Return 1 is muted. In Send, the LED lights when selected-track send 1 is disabled (`mOn` = 0); in Pan, Scroll/Zoom, Section and Marker it lights when the shared track-fader bypass is active; in Channel, it follows the high-pass filter's `mLowCutOn`; in Click, it follows the metronome's `mMetronomeActive`. In Send, Pan, Scroll/Zoom, Section and Marker, **BYPASS LED on means bypassed**. In Channel and Click, LED on means enabled. In normal Master, it means Main Mix inserts are active; in SHIFT + Master, it means FX Return 1 is muted. Feedback follows Cubase edits, track selection, and mode changes; the LED is off in other modes. Send BYPASS toggles the send to the FX destination, not the FX return channel or its inserts.

Outside Master modes, the Master mode LED follows insert slot 1 on the first output channel: solid on when active and off when bypassed. In both Master modes it always flashes, regardless of insert state. The insert feedback reports slot 1 even though the normal Master BYPASS command targets all Main Mix inserts. Normal Master and SHIFT + Master use separate BYPASS routes: Main Mix insert bypass and FX Return mute, respectively.

With `ENABLE_PAN_COLOR = true` (default), active Pan is white at exact center. Moving off center starts at 50% blue (left) or magenta (right), then a square-root curve increases the blend to full color at either edge. Set it to `false` for solid white. Color follows selected-track pan changes in Cubase. The active Send mode shows an amber PAN LED for post-fader or cyan for pre-fader, dim when disabled and bright when enabled; Scroll is white; active Channel uses the high-pass colors below. The other buttons in this four-button group pulse red while recording when `ENABLE_NUCLEAR_RECORD_BLINK = true` (the default), regardless of metronome state. Set `ENABLE_NUCLEAR_METRONOME_LEDS = true` to also show solid `METRONOME_PULSE_COLOR` (blue by default) when the metronome is enabled; this option defaults to `false`. Recording indication takes priority. Otherwise, inactive buttons are off. In Master, Click, Section or Marker mode, these options apply to all four buttons. Neither flag changes the BYPASS LED or the transport RECORD LED. Section and Marker LEDs indicate their active modes. Outside Master modes, the Master LED follows Main Mix insert 1 on/off state; in either Master mode it flashes regardless of insert state. Click flashes for its active mode and otherwise displays the global metronome state.
...
In Scroll/Zoom, Section, and both Master modes, knob push runs `Zoom > Zoom to Locators`, once per press. Scroll/Zoom, Section, Marker, Click, and both Master modes share horizontal zoom rotation. Click knob push still toggles the metronome. Zoom pulses commands for repeated movement and seeds its comparison value on mode entry to avoid an immediate zoom jump. Like the Korg pattern, reaching the normalized range endpoint may require reversing the knob before further travel is available; verify the relative encoder behavior in Cubase.

Channel mode uses Cubase’s Pre section Low Cut (high-pass) frequency and on/off controls. Selecting Channel does not enable the filter automatically; press BYPASS to toggle it. Knob push resets cutoff to minimum without changing enable state or slope. Cutoff adjustment leaves the current enable state and slope intact. Outside High Pass mode, the Channel LED follows the nuclear LED flags; it is white when that mode is active with the filter disabled, and a cutoff-dependent color when enabled. In normal Master, BYPASS toggles Main Mix insert bypass; in SHIFT + Master, BYPASS toggles FX Return 1 mute. Marker mode uses Prev/Next to locate the previous/next marker and encoder push to insert one; encoder rotation controls horizontal zoom. Cubase 12 and 13+ use different command categories for marker insertion, which the script selects based on API feature availability.
...
- With either SHIFT state, Click selects its mode; knob rotation controls horizontal zoom, while encoder push toggles Cubase's metronome. The fader controls metronome click level when `ENABLE_METRONOME_FADER = true`.
...
Calibration uses two straight segments that preserve both travel endpoints. Set FADER_HOST_UNITY to match Cubase's volume range (the original script uses 0.789087 for +6 dB and 0.748222 for +12 dB). Set FADER_HARDWARE_UNITY to the measured normalized raw input at your unit's printed U mark. Its default equals the host setting and makes no correction. Calibration and bottom snap are intended for volume mappings; leave them off for arbitrary parameters. Pan and Channel assign selected-track volume; Send assigns Send 1 level; Master assigns Stereo Out; Click assigns metronome level (Stereo Out when `ENABLE_METRONOME_FADER = false`); Scroll, Section and Marker select track volume.
...
- `node tests_iostation/master-mode.test.js` checks separate Master and SHIFT + Master BYPASS actions, global Main Mix insert LED feedback/blinking, and FX Return mute feedback.
- `node tests_iostation/click-zoom.test.js` checks that Click knob turns route to horizontal zoom and do not edit the metronome level.
