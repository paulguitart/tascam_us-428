...
| Channel | Selected-track high-pass cutoff; knob push toggles the filter; BYPASS toggles phase/polarity |
...
In normal Master, the BYPASS LED follows `mBypass` for insert slot 1 (`accessSlotAtIndex(0)`) through the `Stereo Out Insert 1` viewer on the first output channel. It lights when that slot is **not bypassed**. This reports slot 1's state, while the button command targets all Main Mix inserts. In SHIFT + Master, the BYPASS LED follows the first FX channel's `mMute` and lights when FX Return 1 is muted. In Send, the LED lights when selected-track send 1 is disabled (`mOn` = 0); in Pan, Scroll/Zoom, Section and Marker it lights when the shared track-fader bypass is active; in normal Channel, it follows phase/polarity and lights when polarity is inverted; in Click, it follows the metronome's `mMetronomeActive`. SHIFT + Channel leaves BYPASS off and unassigned. In Send, Pan, Scroll/Zoom, Section and Marker, **BYPASS LED on means bypassed**. In Click, LED on means enabled. In normal Master, it means Main Mix inserts are active; in SHIFT + Master, it means FX Return 1 is muted. Feedback follows Cubase edits, track selection, and mode changes; the LED is off in other modes. Send BYPASS toggles the send to the FX destination, not the FX return channel or its inserts.
...
The active Channel LED uses two simple states per mode. In High Pass mode, white means the filter is off and red means it is on. In SHIFT + Channel / Pre Gain mode, white means exactly 0 dB and magenta means any other gain. Host edits and selected-track changes update the LED. The Channel LED follows the nuclear LED flags while neither Channel mode is active.
...
SHIFT + Channel selects selected-track pre-gain on the knob. The fader remains selected-track volume, and navigation follows the existing SHIFT layer. Channel is white at exactly 0 dB and magenta at any other value. Knob push resets pre-gain to 0 dB. BYPASS is unassigned in Pre Gain mode; phase/polarity is controlled by BYPASS in normal Channel mode, where its LED lights when inverted. Host edits and track selection update both LEDs. While either Channel mode is active, pressing CHANNEL toggles High Pass ↔ Pre Gain, and pressing SHIFT switches immediately to the matching mode. SHIFT is off for High Pass and on for Pre Gain, including when restored through another button’s mode history. Channel uses this two-mode toggle instead of previous-mode recall. Other modes retain their existing SHIFT behavior.
