...
All mode buttons except Channel select and toggle the same modes with SHIFT on or off. SHIFT + Channel selects Pre Gain; unshifted Channel selects High Pass. Changing SHIFT alone does not change the current mode.
...
SHIFT + Channel selects selected-track pre-gain on the knob. The fader remains selected-track volume, and navigation follows the existing SHIFT layer. Channel is white at 0 dB, blending continuously toward blue below zero and magenta above zero, reaching full color at the gain range endpoints. Knob push resets pre-gain to 0 dB. BYPASS toggles polarity inversion, with its LED on when inverted; this works with SHIFT on or off in Pre Gain mode. Host edits and track selection update both LEDs. Changing SHIFT alone retains the current mode; the existing active-button previous-mode behavior also applies to Pre Gain.
