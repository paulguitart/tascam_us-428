...
- `TOUCH_GLOW_LOW_COLOR = AMBER`: below-unity color, including PreGain.
...
Colors accept the named color constants or RGB arrays such as `[127, 48, 0]`. Only brightness changes along each ramp; unity stays full white. Volume ramps account for the unequal travel above and below unity. LINK colors and behavior are unaffected. `node tests_iostation/touch-glow.test.js` verifies custom colors, ramps, disabled options, mandatory PreGain glow and LINK isolation.
