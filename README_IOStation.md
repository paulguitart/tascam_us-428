...
- Encoder rotation uses the knob modes below. In either Master mode it controls horizontal zoom. Encoder push centers pan in Pan mode, toggles Send 1 pre/post-fader in SHIFT + Pan mode, Cubase's metronome in Click mode, resets high-pass cutoff to minimum in Channel mode, toggles Main Mix insert bypass in either Master mode, and inserts a marker in Marker mode.
...
| Master | Encoder controls horizontal zoom; fader controls Stereo Out; encoder push or BYPASS toggles Main Mix inserts; Prev/Next nudge Stereo Out (set left/right locators when nudging is disabled) |
| SHIFT + Master | Encoder controls horizontal zoom; fader controls FX Return 1; encoder push or BYPASS toggles Main Mix inserts; Prev/Next retain Master navigation |
...
In both Master modes, the BYPASS LED follows `mBypass` for insert slot 1 (`accessSlotAtIndex(0)`) through the `Stereo Out Insert 1` viewer on that first output channel. It lights when that slot is **not bypassed**. This reports slot 1's state, while the button command targets all Main Mix inserts. In Send, the LED lights when selected-track send 1 is disabled (`mOn` = 0); in Pan, Scroll/Zoom, Section and Marker it lights when the shared track-fader bypass is active; in Channel, it follows the high-pass filter's `mLowCutOn`; in Click, it follows the metronome's `mMetronomeActive`. In Send, Pan, Scroll/Zoom, Section and Marker, **BYPASS LED on means bypassed**. In Channel, Click, and either Master mode, LED on means enabled. Feedback follows Cubase edits, track selection, and mode changes; the LED is off in other modes. Send BYPASS toggles the send to the FX destination, not the FX return channel or its inserts.
...
All mode buttons except Channel, Pan and Master select and toggle the same modes with SHIFT on or off. SHIFT + Channel selects Pre Gain; unshifted Channel selects High Pass. Changing SHIFT switches between High Pass and Pre Gain when either Channel mode is active, and between Stereo Out and FX Return 1 fader targets in Master; other modes stay selected. SHIFT + Master's encoder still zooms.

In Scroll/Zoom and Section modes, knob push runs `Zoom > Zoom to Locators`, once per press. Scroll/Zoom, Section, Marker, and both Master modes share horizontal zoom rotation. Master knob push remains assigned to Main Mix insert bypass. Zoom pulses commands for repeated movement and seeds its comparison value on mode entry to avoid an immediate zoom jump. Like the Korg pattern, reaching the normalized range endpoint may require reversing the knob before further travel is available; verify the relative encoder behavior in Cubase.

Channel mode uses Cubase’s Pre section Low Cut (high-pass) frequency and on/off controls. Selecting Channel does not enable the filter automatically; press BYPASS to toggle it. Knob push resets cutoff to minimum without changing enable state or slope. Cutoff adjustment leaves the current enable state and slope intact. Outside High Pass mode, the Channel LED follows the nuclear LED flags; it is white when that mode is active with the filter disabled, and a cutoff-dependent color when enabled. In both Master modes, encoder push runs Cubase's `Mixer > Bypass: Inserts on Main Mix` command. Marker mode uses Prev/Next to locate the previous/next marker and encoder push to insert one; encoder rotation controls horizontal zoom. Cubase 12 and 13+ use different command categories for marker insertion, which the script selects based on API feature availability. Section knob push zooms to locators.
...
| Master | FX Return 1 fader with zoom knob; push and BYPASS still toggle Main Mix inserts |
| Click / Section / Marker | Same modes as without SHIFT |

Assign future commands to unused non-mode paths such as `buttons.Trim`. These are logical surface values that receive press/release events. Solo, Mute, Arm, Write, Read, Prev, Next, Undo and Redo have Cubase assignments; Link, Pan, Scroll, Zoom, Master, MasterFX, Click, Channel, Section and Marker select knob modes. Encoder push centers pan in Pan mode, resets the first send to 0 dB in SHIFT + Pan mode, toggles the metronome in Click mode, resets cutoff to minimum in Channel mode, toggles Main Mix insert bypass in both Master modes, and inserts a marker in Marker mode. SHIFT + Solo runs `Edit > Deactivate All Solo`; SHIFT + Mute runs `Edit > Unmute All`. SHIFT + Arm alternates `Mixer > Arm All Audio Tracks` and `Mixer > Disarm All Audio Tracks`, following Werner�s selected-track arm feedback: an armed selected track sets the next action to Disarm All, and an unarmed one sets it to Arm All. This targets audio tracks and does not inspect the arm state of every track. Other named paths remain unassigned. The printed Lock, Flip and F1-F4 labels do not have separate actions. The visible physical controls retain their original layout and primary labels.
...
- `node tests_iostation/knob.test.js` checks knob and fader mode targets, SHIFT + Master FX Return 1 fader binding, insert-bypass push in both Master modes, selectors, mode LEDs, marker controls, repeated zoom pulses and mode-entry baselines.
