...
- Encoder rotation uses the knob modes below. In Click and either Master mode it controls horizontal zoom. Encoder push centers pan in Pan mode, toggles Send 1 pre/post-fader in SHIFT + Pan mode, zooms to locators in Click and either Master mode, resets high-pass cutoff to minimum in Channel mode, and inserts a marker in Marker mode.
