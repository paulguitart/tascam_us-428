...
| Channel | Selected-track high-pass cutoff; knob push toggles the filter; BYPASS toggles phase/polarity in either Channel mode |
...
The active Channel LED uses two simple states per mode. In High Pass mode, white means the filter is off and red means it is on. In SHIFT + Channel / Pre Gain mode, white means exactly 0 dB and amber means any other gain. Host edits and selected-track changes update the LED. The Channel LED follows the nuclear LED flags while neither Channel mode is active.
...
SHIFT + Channel puts selected-track Pre-section gain (`mPreFilter.mGain`) on the fader, preserving the full -infinity to +48 dB range. Process values 0, 0.5 and 1 map directly to bottom, physical center (0 dB) and top. Volume unity calibration and low-end snapping do not apply to this layer. Navigation follows the existing SHIFT layer. Channel is white at exactly 0 dB and amber at any other value. TOUCH resets pre-gain to 0 dB; host feedback drives the motor and TOUCH LED (white at zero, amber at minimum, red at maximum, off otherwise). The knob steps `mPreFilter.mLowCutSlope` through 6, 12, 24, 36 and 48 dB/oct; push resets to 12 dB/oct. Cubase converts display choices to process values, and display feedback records those values without guessed normalized constants. Rotation uses relative direction and current host display feedback, without recentering either encoder value; the next detent after reset selects 6 or 24. BYPASS toggles phase/polarity in Pre Gain mode as well as normal Channel mode, with its LED on when inverted. Host edits and track selection update both LEDs. While either Channel mode is active, pressing CHANNEL toggles High Pass ↔ Pre Gain, and pressing SHIFT switches immediately to the matching mode. SHIFT is off for High Pass and on for Pre Gain, including when restored through another button’s mode history. Channel uses this two-mode toggle instead of previous-mode recall. Other modes retain their existing SHIFT behavior.
...
Cubase/hardware verification still required: confirm -infinity / 0 dB / +48 dB at bottom / center / top, motor and LEDs after TOUCH and track selection, and all five slope display conversions including immediate detents in both directions after push. Confirm repeated relative detents at encoder endpoints, and normal CHANNEL volume/cutoff/filter toggle/phase behavior. API-free tests simulate host feedback; they cannot establish actual Cubase enum process values. See the [MIDI Remote API](https://steinbergmedia.github.io/midiremote_api_doc/codedoc_api_reference/) and [Cubase filter settings](https://www.steinberg.help/r/cubase-pro/15.0/en/cubase_nuendo/topics/mixconsole/mixconsole_making_filter_settings_t.html).
