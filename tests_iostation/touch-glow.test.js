...
const s={ENABLE_VOLUME_TOUCH_GLOW:true,ENABLE_CLICK_TOUCH_GLOW:true,TOUCH_GLOW_LOW_COLOR:[1,2,3],TOUCH_GLOW_HIGH_COLOR:[4,5,6],TOUCH_CLICK_GLOW_COLOR:[7,8,9],TOUCH_GLOW_MIN_BRIGHTNESS:.03,FADER_HOST_UNITY:.75,
...
function check(target,v,c,b,on=true){state.faderTarget=target;level=v;s.faderTargetFeedback[target]={getProcessValue:()=>level};s.updateTouchLED(ctx);assert.equal(lit,on);if(on){assert.deepEqual(color,c);assert(Math.abs(brightness-b)<1e-10)}}
for(const target of ['Track','Send','StereoOut','FXReturn','PreGain']){
 const u=target==='PreGain'?.5:.75;
 for(const [v,c,b] of [[0,[1,2,3],1],[u/2,[1,2,3],.515],[u-.001,[1,2,3],.03+.97*.001/u],[u,[127,127,127],1],[u+(1-u)/2,[4,5,6],.515],[1,[4,5,6],1]])check(target,v,c,b);
}
