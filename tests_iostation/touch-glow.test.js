...
const s={ENABLE_VOLUME_TOUCH_GLOW:true,ENABLE_CLICK_TOUCH_GLOW:true,TOUCH_GLOW_LOW_COLOR:[1,2,3],TOUCH_PREGAIN_LOW_COLOR:[9,10,11],TOUCH_GLOW_BOTTOM_COLOR:[11,22,33],TOUCH_GLOW_HIGH_COLOR:[4,5,6],TOUCH_CLICK_GLOW_COLOR:[7,8,9],TOUCH_GLOW_MIN_BRIGHTNESS:.03,FADER_HOST_UNITY:.75,
...
for(const target of ['Track','Send','StereoOut','FXReturn','PreGain']){
 ...
 for(const fraction of [0,.25,.5,.999]){
  const d=1-fraction;
  const c=target==='PreGain'?[9,10,11]:[1+10*d,2+20*d,3+30*d];
...
check('PreGain',.25,[9,10,11],.515); // Mandatory even with both optional flags off.
