...
s.lowCutSlopeFeedbackValue.setDisplayValue=(c,v)=>{slope=Number(v);assert(slope in process);slopeWrites.push(slope);s.lowCutSlopeFeedbackValue.mOnDisplayValueChange(c,slope+' dB/oct')};
...
push();assert.deepEqual(slopeWrites,[12]);turn(0,-.01);assert.equal(slope,6);
...
console.log('PASS: both Channel fader targets, full-range feedback/motor mapping, reset/LEDs, host-converted slope choices, endless turns after reset, normal CHANNEL calibration/filter/phase isolation');
